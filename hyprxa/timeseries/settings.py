...
class TimeseriesSettings(BaseSettings):
    ...
    collection_name: str | None = Field(
        ...
        timeseries data. Defaults to {}""".format(TimeseriesWorker.default_collection_name()))
    )
    flush_interval: float = Field(
        default=defaults["flush_interval"].default,
        description=format_docstrings("""Interval between document flushes to the
        database. Defaults to {}""".format(defaults["flush_interval"].default))
    )
    buffer_size: int = Field(
        ...
        """.format(defaults["buffer_size"].default))
    )
    max_retries: int = Field(
        ...
        """.format(defaults["max_retries"].default))
    )
